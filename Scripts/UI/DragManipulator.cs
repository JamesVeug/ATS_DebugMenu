...
public class DragManipulator : IManipulator {
  ...
  public VisualElement target {
    ...
    set {
      ...
      // Walk up parent elements to see if any are a parent we actually want to drag.
      var element = value;
      while (element != null && ! element.ClassListContains("draggable-parent"))
        element = element.parent;

      _dragElement = element ?? value;
      _target = value;
  ...
  private void DragEnd(IPointerEvent ev) {
    ...
    if (canDrop)
      Drop(droppable);
    else if(target.ClassListContains("reset-on-droppping-done"))
      ResetPosition();
  ...
  public virtual void ResetPosition() {
    _dragElement.transform.position = Vector3.zero;
  }
  ...
  private void PointerMove(PointerMoveEvent ev) {
    ...
    _dragElement.transform.position += delta;
